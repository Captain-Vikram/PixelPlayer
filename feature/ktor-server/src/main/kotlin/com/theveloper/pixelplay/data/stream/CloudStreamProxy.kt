...
abstract class CloudStreamProxy<K : Any>(
    ...
    fun register() {
        StreamProxyRegistry.registerResolver(uriScheme) { uri ->
            val uriString = uri.toString()
            if (ensureReady(5_000L)) {
                warmUpStreamUrl(uriString)
                resolveUri(uriString)?.let { Uri.parse(it) }
            } else null
        }
    }

    open suspend fun warmUpStreamUrl(uriString: String) {
        val uri = Uri.parse(uriString)
        if (uri.scheme != uriScheme) return
        val rawId = extractIdFromUri(uri) ?: return
        val id = parseRouteParam(rawId) ?: return
        if (!validateId(id)) return
        try {
            getOrFetchStreamUrl(id)
        } catch (e: Exception) {
            Timber.w(e, "$proxyTag: warmUpStreamUrl failed for $rawId")
        }
    }
                    ...
                    try {
                        ...
                        response.use { upstream ->
                            if (upstream.code != 200 && upstream.code != 206) {
                                call.respond(
                                    HttpStatusCode.fromValue(CloudStreamSecurity.mapUpstreamStatusToProxyStatus(upstream.code)),
