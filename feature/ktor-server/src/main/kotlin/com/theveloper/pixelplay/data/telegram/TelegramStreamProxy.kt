...
class TelegramStreamProxy @Inject constructor(
    ...
    fun register() {
        com.theveloper.pixelplay.data.stream.StreamProxyRegistry.registerResolver("telegram") { uri ->
            if (!telegramRepository.isReady()) {
                telegramRepository.awaitReady(10_000L)
            }
            val uriString = uri.toString()
            val resolved = telegramRepository.resolveTelegramUri(uriString)
            val fileId = resolved?.first
                ?: uri.host?.toIntOrNull()
                ?: uri.pathSegments.firstOrNull()?.toIntOrNull()
            
            if (fileId != null && ensureReady(5_000L)) {
                val knownSize = (resolved?.second ?: 0L).coerceAtLeast(0L)
                val proxyUrl = getProxyUrl(fileId, knownSize)
                if (proxyUrl.isNotEmpty()) android.net.Uri.parse(proxyUrl) else null
            } else null
        }
    }
        ...
        return embeddedServer(CIO, port = port, host = "127.0.0.1") {
            routing {
                get("/stream/{fileId}") {
                    ...
                    var fileInfo = telegramRepository.downloadFile(fileId, 1) as? org.drinkless.tdlib.TdApi.File
                    ...
                    while (fileInfo?.local?.path.isNullOrEmpty() && pathWaitCount < 50) { // Wait up to 2.5s
                        delay(50)
                        fileInfo = telegramRepository.getFile(fileId) as? org.drinkless.tdlib.TdApi.File
                        pathWaitCount++
                    }
                        ...
                        try {
                            ...
                            while (true) {
                                ...
                                if (currentPos >= cachedDownloadedPrefixSize) {
                                     val updatedInfo = telegramRepository.getFile(fileId) as? org.drinkless.tdlib.TdApi.File
