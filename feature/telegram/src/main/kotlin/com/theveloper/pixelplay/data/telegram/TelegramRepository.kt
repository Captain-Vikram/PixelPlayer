...
class TelegramRepository @Inject constructor(
    ...
    private val playlistPreferencesRepository: PlaylistRepositoryContract
) : com.theveloper.pixelplay.data.repository.TelegramRepositoryContract {
    ...
    override val isAuthorizedFlow: Flow<Boolean> = clientManager.authorizationState.map {
        it is TdApi.AuthorizationStateReady
    }
    ...
    override fun isReady(): Boolean = clientManager.isReady()
    ...
    override suspend fun downloadFile(fileId: Int, priority: Int): TdApi.File? {
        return try {
    ...
    override suspend fun getDownloadedFilePath(fileId: Int): String? {
        val fileInfo = getFile(fileId)
        if (fileInfo != null && fileInfo.local.isDownloadingCompleted && fileInfo.local.path.isNotEmpty()) {
    ...
    override suspend fun resolveTelegramUri(uriString: String): Pair<Int, Long>? {
