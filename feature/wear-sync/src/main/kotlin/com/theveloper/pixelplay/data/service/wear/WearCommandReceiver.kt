...
import com.theveloper.pixelplay.data.repository.PlaylistRepositoryContract
...
import com.theveloper.pixelplay.data.repository.WearMusicRepositoryContract
...
class WearCommandReceiver : WearableListenerService() {
    ...
    @Inject lateinit var musicRepository: WearMusicRepositoryContract
    @Inject lateinit var playlistRepository: PlaylistRepositoryContract
    ...
    companion object {
        ...
        var resolveMediaItemCallback: (suspend (MediaItem) -> MediaItem)? = null
        var buildMediaItemCallback: ((Context, Song) -> MediaItem)? = null
    }
    ...
    private fun handlePlayFromContext(command: WearPlaybackCommand) {
        ...
        scope.launch {
            try {
                ...
                val mediaItems = songs.mapNotNull { buildMediaItemCallback?.invoke(this@WearCommandReceiver, it) }.toMutableList()
                val startIndex = songs.indexOfFirst { it.id == songId }.coerceAtLeast(0)
                if (mediaItems.isNotEmpty()) {
                    mediaItems[startIndex] = resolveMediaItemCallback?.invoke(mediaItems[startIndex]) ?: mediaItems[startIndex]
                }
    ...
    private suspend fun getSongsForContext(contextType: String, contextId: String?): List<Song> {
        return when (contextType) {
            ...
            "playlist" -> {
                ...
                val playlist = playlistRepository.userPlaylistsFlow.first()
    ...
    private suspend fun getBrowseItems(browseType: String, contextId: String?): List<WearLibraryItem> {
        return when (browseType) {
            ...
            WearBrowseRequest.PLAYLISTS -> {
                playlistRepository.userPlaylistsFlow.first()
                    .map { playlist ->
                        WearLibraryItem(
                            id = playlist.id,
                            title = playlist.name,
    ...
    private fun getOrBuildMediaController(action: (MediaController) -> Unit) {
        runOnMainThread {
            ...
            val sessionToken = SessionToken(
                this,
                ComponentName(this, "com.theveloper.pixelplay.data.service.MusicService")
            )
