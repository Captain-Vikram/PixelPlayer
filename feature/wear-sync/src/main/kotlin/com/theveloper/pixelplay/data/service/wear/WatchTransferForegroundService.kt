...
class WatchTransferForegroundService : Service() {
    ...
    private fun getResString(name: String, vararg formatArgs: Any?): String {
    ...
    private fun getResDrawable(name: String): Int {
    ...
    private fun buildNotification(transfers: List<PhoneWatchTransferState>): Notification {
        ...
        val title = when {
            activeTransfers.size > 1 -> getResString("watch_transfer_status_sending_n_to_watch", activeTransfers.size)
            activeTransfers.size == 1 -> getResString("watch_transfer_status_sending_to_watch")
            selectedTransfer?.status == WearTransferProgress.STATUS_COMPLETED -> getResString("watch_transfer_status_complete_service")
            selectedTransfer?.status == WearTransferProgress.STATUS_FAILED -> getResString("watch_transfer_status_failed_service")
            selectedTransfer?.status == WearTransferProgress.STATUS_CANCELLED -> getResString("watch_transfer_status_cancelled_service")
            else -> getResString("watch_transfer_status_preparing_service")
        }
        ...
        val builder = NotificationCompat.Builder(this, NOTIFICATION_CHANNEL_ID)
            .setSmallIcon(getResDrawable("monochrome_player"))
        ...
        if (transfers.size > 1) {
            val style = NotificationCompat.InboxStyle()
                .setSummaryText(getResString("watch_transfer_summary_n", transfers.size))
            transfers
    ...
    private fun buildContentText(
        ...
        if (transfer == null) return getResString("watch_transfer_status_starting")

        if (activeTransferCount > 1) {
            return transfer.songTitle.ifBlank { getResString("watch_transfer_status_multiple_active") }
        }

        val songTitle = transfer.songTitle.ifBlank { getResString("watch_transfer_status_preparing_transfer") }
    ...
    private fun buildDetailedText(transfer: PhoneWatchTransferState?): String {
        ...
        val statusLine = when (transfer.status) {
            WearTransferProgress.STATUS_TRANSFERRING -> getResString("watch_transfer_status_transferring")
            WearTransferProgress.STATUS_COMPLETED -> getResString("watch_transfer_status_completed")
            WearTransferProgress.STATUS_FAILED -> getResString("watch_transfer_status_failed")
            WearTransferProgress.STATUS_CANCELLED -> getResString("watch_transfer_status_cancelled")
            else -> getResString("watch_transfer_status_preparing")
        }
    ...
    private fun formatTransferLine(transfer: PhoneWatchTransferState): String {
        ...
        val status = when (transfer.status) {
            WearTransferProgress.STATUS_TRANSFERRING -> {
                ...
                if (transfer.totalBytes > 0L) "$percent%" else getResString("watch_transfer_status_starting_short")
            }
            WearTransferProgress.STATUS_COMPLETED -> getResString("watch_transfer_status_completed")
            WearTransferProgress.STATUS_FAILED -> getResString("watch_transfer_status_failed")
            WearTransferProgress.STATUS_CANCELLED -> getResString("watch_transfer_status_cancelled")
    ...
    private fun createOpenAppPendingIntent(): PendingIntent {
        ...
        val intent = if (mainActivityClass != null) Intent(this, mainActivityClass) else Intent()
        intent.apply {
    ...
    private fun ensureNotificationChannel() {
        ...
        val channel = NotificationChannel(
            ...
            getResString("watch_transfer_channel_name"),
            NotificationManager.IMPORTANCE_LOW,
        ).apply {
            description = getResString("watch_transfer_channel_description")
            setShowBadge(false)
        }
