...
class PhoneDirectWatchTransferCoordinator @Inject constructor(
    ...
    private val musicRepository: WearMusicRepositoryContract,
    ...
    private suspend fun resolveStreamUrl(song: Song): String? {
        ...
        return when (scheme) {
            "http", "https" -> rawUri
                ...
                if (StreamProxyRegistry.hasResolver(scheme)) {
                    StreamProxyRegistry.resolve(scheme, uri)?.toString()
                } else null
            }
        }
    }
    ...
    private suspend fun resolveTransferThemePalette(song: Song): WearThemePalette? {
        return resolveTransferThemePaletteCallback?.invoke(song)
    }
    ...
    private fun decodeBoundedBitmapFromUri(uriString: String): Bitmap? {
        ...
        openArtworkInputStreamCallback?.invoke(application, uri)?.use { stream ->
        ...
        return openArtworkInputStreamCallback?.invoke(application, uri)?.use { stream ->
    ...
    private fun loadSongAlbumArtBitmap(song: Song): Bitmap? {
        val artFromUri = song.albumArtUriString
            ...
            ?.let { uriString ->
                runCatching {
                    openArtworkInputStreamCallback?.invoke(application, uriString.toUri())?.use { input ->
    ...
    companion object {
        ...
        var openArtworkInputStreamCallback: ((Context, Uri) -> java.io.InputStream?)? = null
        var resolveTransferThemePaletteCallback: (suspend (Song) -> WearThemePalette?)? = null
    }
}
